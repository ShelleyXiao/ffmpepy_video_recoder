...
int fill_pcm_frame_callback(int16_t *samples, int frame_size, int nb_channels,
                            double *prsentationTimeMills, void *context) {
    AudioEncoderAdapter *audioEncoderAdapter = (AudioEncoderAdapter *) context;
    audioEncoderAdapter->getAudioFrame(samples, frame_size, nb_channels, prsentationTimeMills);
}

void
AudioEncoderAdapter::init(LivePacketPool *pcmPacketpool, int audioSampleRate, int aduioChannles,
    ...
    this->codecName = new char[audioCodecNameLength + 1];
    memcpy(this->codecName, audio_codec_name, audioCodecNameLength);
...
void AudioEncoderAdapter::distroy() {
    isEncoding = false;
...
void AudioEncoderAdapter::startEncode() {
    ...
    audioEncoder->init(this->audioBitRate, this->audioChannels, this->audioSampleRate,
                       this->codecName, fill_pcm_frame_callback, this);
    while (isEncoding) {
...
int AudioEncoderAdapter::cpyToSamples(int16_t *samples, int samplesInShortCursor,
                                      int cpyPacketBufferSize, double *presentationTimeMills) {
    if (0 == samplesInShortCursor) {
        //第一次为samples拷贝进sample 这里要记录时间给sampleBuffer
        double packetBufferCursorDuration =
                (double) packetBufferCursor * 1000.0f / (double) (audioSampleRate * channelRatio);
        (*presentationTimeMills) = packetBufferPresentationTimeMills + packetBufferCursorDuration;
    }
    memcpy(samples + samplesInShortCursor, packetBuffer + packetBufferCursor,
           cpyPacketBufferSize * sizeof(short));
    return 1;
}
...
int AudioEncoderAdapter::getAudioFrame(int16_t *samples, int frame_size, int nb_channels,
    ...
    while (true) {
        ...
        if (packetBufferCursor + copyToSamplesInShortSize <= packetBufferSize) {
            this->cpyToSamples(samples, samplesInShortCursor, copyToSamplesInShortSize,
                               presentationTimeMills);
            packetBufferCursor += copyToSamplesInShortSize;
            samplesInShortCursor = 0;
            break;
        } else {
            int suPacketBufferSize = packetBufferSize - packetBufferCursor;
            this->cpyToSamples(samples, samplesInShortCursor, suPacketBufferSize,
                               presentationTimeMills);
            samplesInShortCursor += suPacketBufferSize;
            packetBufferSize = 0;
            continue;
        }
